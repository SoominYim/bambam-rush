...
## 중복 컨셉 금지 (강제)
- 기존 무기와 컨셉이 겹치는 제안은 거절한다.
- 아래 3축 중 2개 이상 겹치면 중복으로 본다.
  - 공격 루프(예: 투사체 직선 왕복, 오라 지속 틱)
  - 체감/조작감(예: 자동 추적, 설치형, 회전형)
  - 핵심 판정 방식(예: 단일 관통, 범위 폭발, 체인 전이)
- 신규 무기는 반드시 `기존 유사 무기 대비 차별점 3개`를 명시한다.
- 초반 단독 무기 구간에서 즉시 공격 성립이 안 되면 채택하지 않는다.
...
## 에셋 브리프 연동 (필수)
- 신규/리워크 무기는 `docs/WEAPON_ASSET_BRIEF.md`에 항목을 추가하거나 기존 항목을 갱신한다.
- 아래 5필드가 비어 있으면 구현 완료로 보지 않는다.
  - `Silhouette`
  - `Material/Texture`
  - `Primary Color`
  - `VFX Keyword`
...
- [ ] 툴팁/문서 동기화
- [ ] 기존 무기 중복 검토표 작성(유사 무기/차별점/초반 성립성)
- [ ] `docs/WEAPON_ASSET_BRIEF.md` 5필드 검증
