...
## GitHub 작업 규칙 (PR/머지)
- PR 요청 및 본문에 한국어 요약을 필수로 작성한다.
- PR 제목은 기본 형식: `[도메인] #<이슈번호> 간단 요약`  
  예: `[무기] #17 샷건 사거리/두께 조정`.
- PR 본문 최상단에 `## 변경 요약`을 넣고, 본문 내에 아래 항목을 포함한다.
  - 변경 파일
  - 변경 이유
  - 기대 효과
  - 체크리스트(빌드/타입/린트)
- PR 설명에는 이슈/카드 번호를 반드시 넣는다.  
  - 적어도 하나의 번호는 `#숫자` 형태로 명시.
  - 번호가 없다면 `#2` 같은 임시 placeholder를 최소 하나라도 사용해 추적 기준 확보.
- 머지 전 확인:
  - 리뷰 코멘트가 있을 경우 모두 반영 확인
  - 체크리스트 항목(타입/린트/빌드) 통과
  - 본문 이슈 번호와 실제 변경 내용 일치
- 머지 커밋은 `main`에 바로 반영 전, 변경 이유와 영향 범위를 커밋 메시지 본문에 1줄 이상 적는다.
- 댓글/리뷰/리포트는 기본적으로 한국어로 작성한다.
...
## 에이전트 운영 목표 (브랜치/PR/리뷰/머지)
- 기본 목표: `브랜치 생성 -> 원격 푸시 -> PR 생성 -> 리뷰 코멘트 반영 -> 머지`를 완료한다.
- PR 코멘트는 actionable 항목을 전부 처리하고, 처리 내역을 PR에 남긴다.
- 머지 전에는 최소 `lint/typecheck/build` 중 가능한 검증을 수행한다.
- 작업 완료 보고에는 `변경 파일/핵심 변경/검증 결과/남은 리스크`를 포함한다.
