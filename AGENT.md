...
## 1) 핵심 목표
- 기본 개발 사이클: `브랜치 생성 -> 구현/검증 -> 원격 푸시 -> PR 생성`
- 기본 작업 종료 지점은 `PR 생성 완료`다.
- `머지`는 사용자의 명시적 지시가 있을 때만 수행한다.
...
- 아래 순서를 고정으로 따른다.
1. 코드 수정 끝
2. 로컬 검증(`lint/build` 등) 끝
3. 브랜치 푸시
4. PR 생성
5. 코드리뷰 코멘트 작성/등록 (`feat/fix/chore/docs` 구분 없이 필수)
6. 리뷰 반영 커밋 추가
7. 다시 리뷰 확인
...
- 커밋 메시지는 `타입 접두어`를 제외하고 한글로 작성한다.
  - 예시: `feat: 무기 표식 연계 폭발 로직 추가`, `chore: 작업 절차 규칙 문서화`
